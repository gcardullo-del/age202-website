...
type ArtifactFormAction = (formData: FormData) => void | Promise<void>;
...
const sections = [
  { id: "general", label: "General", description: "Identity and classification", icon: Landmark },
  { id: "media", label: "Media", description: "Cover and gallery", icon: Images },
  { id: "story", label: "Story", description: "Museum narrative", icon: BookOpenText },
  { id: "marketplace", label: "Marketplace", description: "Availability and price", icon: ShoppingBag },
  { id: "authenticity", label: "Authenticity", description: "Codes and attributes", icon: BadgeCheck },
  { id: "publication", label: "Publication", description: "Status and visibility", icon: FileText },
] as const;
...
export default function ArtifactForm({
  ...
  if (isEditing && !artifactId) {
    throw new Error("artifactId is required when ArtifactForm is used in edit mode.");
  }

  const formAction = action ?? (isEditing ? updateArtifact : createArtifact);
  const previewHref = artifactId ? `/product/${artifactId}` : null;

  return (
    <form action={formAction} className="pb-8">
      {artifactId && <input type="hidden" name="artifactId" value={artifactId} />}
      ...
      <div className="mb-6 overflow-hidden rounded-3xl border border-white/10 bg-white/[0.025]">
          ...
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <span className="rounded-full border border-lime-300/20 bg-lime-300/10 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-lime-200">
                {isEditing ? "Artifact workspace" : "New catalog entry"}
              </span>
            ...
            <h2 className="mt-4 text-2xl font-semibold text-white">
              {initialValues?.title || (isEditing ? "Edit artifact" : "Create artifact")}
            </h2>
        ...
        <div className="grid lg:grid-cols-[260px_minmax(0,1fr)]">
          <nav className="border-b border-white/10 p-3 lg:border-b-0 lg:border-r lg:p-4" aria-label="Artifact sections">
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-1">
              {sections.map((section) => {
                ...
                return (
                  ...
                  >
                    <span className={`grid h-9 w-9 shrink-0 place-items-center rounded-xl ${isActive ? "bg-black/10" : "bg-white/5"}`}>
                      <Icon className="h-4 w-4" />
                    </span>
                    <span className="min-w-0">
                      <span className="block truncate text-sm font-semibold">{section.label}</span>
                      <span className={`hidden truncate text-xs lg:block ${isActive ? "text-[#050B18]/60" : "text-white/30"}`}>
                        {section.description}
                      </span>
                    </span>
                  </button>
          ...
          <div className="min-w-0 p-4 sm:p-6 lg:p-8">
            <SectionPanel active={activeSection === "general"}>
              <GeneralInfoCard
                players={players}
                brands={brands}
                initialValues={{
                  ...
                  archiveNumber: initialValues?.archiveNumber ?? "",
                  year: initialValues?.year ?? undefined,
                  season: initialValues?.season ?? "",
                  tournament: initialValues?.tournament ?? "",
                  ...
                  condition: initialValues?.condition ?? "EXCELLENT",
            ...
            <SectionPanel active={activeSection === "media"}>
              <MediaCard existingImages={initialValues?.images ?? []} />
            </SectionPanel>

            <SectionPanel active={activeSection === "story"}>
              <StoryCard
                initialValues={{
                  description: initialValues?.description ?? "",
                  museumStory: initialValues?.museumStory ?? "",
                  historicalContext: initialValues?.historicalContext ?? "",
                  curatorNote: initialValues?.curatorNote ?? "",
                }}
              />
            </SectionPanel>

            <SectionPanel active={activeSection === "marketplace"}>
              <MarketplaceCard
                initialValues={{
                  availability: initialValues?.availability ?? "COMING_SOON",
                  price: initialValues?.price ?? null,
                  currency: initialValues?.currency ?? "EUR",
                  vintedUrl: initialValues?.vintedUrl ?? "",
                }}
              />
            </SectionPanel>

            <SectionPanel active={activeSection === "authenticity"}>
              <AuthenticityCard
                authentic={initialValues?.authentic ?? false}
                authenticityCode={initialValues?.authenticityCode ?? ""}
                vintage={initialValues?.vintage ?? false}
                tags={initialValues?.tags ?? []}
              />
            </SectionPanel>

            <SectionPanel active={activeSection === "publication"}>
              <PublicationCard
                initialValues={{
